import React, { useRef, useContext } from 'react';
import { Link, useParams } from 'react-router-dom';
import { ApplicationContext } from '../Context';
import Cookies from 'js-cookie'
import { TextField, Grid, Icon, FormControl, Input, FormHelperText } from '@mui/material';
import { useNavigate } from 'react-router-dom';
import icons from '@ant-design/icons'
import { Modal, Button, Form } from 'antd';
import { Table, Tag, Space } from 'antd';
import FormProduct from './FormProduct';
import { useDispatch, useSelector } from 'react-redux'
import * as actions from '../actions'
import * as logicProduct  from '../bussiness/logicFormProduct'
import {loadProduct} from '../bussiness/actions/loadProduct'
import { loadProductMissing } from '../bussiness/actions/loadProductMissingAction'
import { URL_PRODUCT_MISSING, URL_PRODUCT_DOWNLOAD_MISSING } from '../bussiness/endpoints';
import ProductMissing from './ProductMissing';

const ListProductMissing = (props) => {
  const value = useContext(ApplicationContext);
  const navigate = useNavigate();
  const { proveedores, parametros } = value;
  const proveedorRef = useRef();
  const [listaProductMissing, setListaProductMissing] = React.useState([]);
  const [visible, setVisible] = React.useState(false);
  const [visibleDelete, setVisibleDelete] = React.useState(false) 
  
  const dispatch = useDispatch()
  const updateDataProduct = useSelector(store => store.editProduct);
  const [form] = Form.useForm()
  const resultUdateProduct = useSelector(store => store.updateProduct)
  const [productDelete, setProductDelete] = React.useState({})
  const INSTANCE_PRODUCT_MISSING = 'instanceListProductMissing';
  
 
  const columns = [
    {
      title: 'Action',
      key: 'action',
      render: (text, record) => (
        <Space size="middle">
          <a onClick={(event) => editarProducto(record.sku)}>Editar</a>
          <a onClick={() => {
              setProductDelete({id: record.id, description: record.description})
              setVisibleDelete(true);
              
          }}>Delete</a>
          <a onClick={(event) => editarProductoMissing(record.sku)}>Faltante</a>
          
        </Space>
      ),
    },
    {
      title: 'Codigo interno',
      dataIndex: 'sku',
      key: 'sku',
      render: text => <p>{text}</p>,
    },
    {
      title: 'Proveedor',
      dataIndex: 'proveedor',
      key: 'proveedor',
    },
    {
      title: 'Codigo proveedor',
      dataIndex: 'codigoProveedor',
      key: 'codigoProveedor',
    },
    {
      title: 'Fecha',
      dataIndex: 'fecha',
      key: 'fecha',
    },
    {
      title: 'Descripcion',
      dataIndex: 'description',
      key: 'codigointerno',
    },
    {
      title: 'Cant. solicitada',
      dataIndex: 'cantidadSolicitada',
      key: 'cantidadSolicitada',
    },
    {
      title: 'Existencia',
      dataIndex: 'cantidadExistencia',
      key: 'cantidadSolicitada',
    }
  ]
 
  React.useEffect( () => {
    if (resultUdateProduct && 'resultUpdate' in resultUdateProduct && resultUdateProduct.resultUpdate.isOk === true) {
      setVisible(false); 
    }
  },[resultUdateProduct])

  const editarProducto = (codigo) => {
    console.log("Nuevo editar producto new implementation 2 = " + codigo)
    dispatch(loadProduct('PURGE'))
    dispatch(loadProduct(codigo))
    setVisible(true); 
  }

  const editarProductoMissing = (codigo) => {
    console.log("Editar producto missing" + codigo)
    dispatch(loadProductMissing('PURGE'))
    dispatch(loadProductMissing(codigo,INSTANCE_PRODUCT_MISSING))
  }

  const handleSearch = (event) => {
    var url = URL_PRODUCT_MISSING + "?supplier=" + proveedorRef.current.value;
    if (proveedorRef.current.value == 0) {
      url = URL_PRODUCT_MISSING;
    }
    console.log("get datos a " + url);
 
    fetch(url,
    {
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json'
        },
        method: "GET"
    })
    .then(response => response.json())
    .then(function(res){
        
        console.log(res.items)
        setListaProductMissing(res.items)

      })
    .catch(function(res){ 
      console.log(res) 
    })

  }

  const downloadMissing = (event) => {
    var url = URL_PRODUCT_DOWNLOAD_MISSING + "?supplier=" + proveedorRef.current.value;
    if (proveedorRef.current.value == 0) {
      url = URL_PRODUCT_DOWNLOAD_MISSING;
    }
    console.log("get datos a " + url);
    window.location.href = url;
  }

  const deleteItem = (id) => {

    var url = URL_PRODUCT_MISSING + "/" + id;
    console.log("delete datos a " + "/" + url);
 
    fetch(url,
    {
        method: 'DELETE',
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json'
        },
        
        
    })
    .then(response => response.json())
    .then(function(res){
        
        console.log(res)
        let newLista = listaProductMissing.filter ( (data) => {
           return data.id != id
        })
        setListaProductMissing(newLista)
        setVisibleDelete(false)

      })
    .catch(function(res){ 
      console.log(res) 
    })

  }

 const actualizarProducto = () => {
   console.log("llamando actionFormProduct con update")
   form.submit()
   console.log("fin ")
 }
      
  
 

  return (
    <div>
      <p>Productos a comprar</p>

        <Grid container spacing={1}>
            <Grid item xs={9}>
              <label htmlFor="proveedor">Proveedor</label>
              <select id="proveedor" ref={proveedorRef} options={proveedores}>
                 <option key={0} value={0}>Todos</option>
                 { proveedores.map ( (proveedor) => (
                   <option key={proveedor.value} value={proveedor.value}>{proveedor.label}</option>
                 ))}
                 
              </select>  
            </Grid>
           
            <Grid item xs={3}>
               <a href="#" onClick={downloadMissing} target='_self'>
                <Button variant="contained"  variant="contained" disabled={false} color="primary" >
                    <Icon>download</Icon>
                </Button>
               </a>
               <Button variant="contained"  variant="contained" onClick={handleSearch} disabled={false} color="primary" >
                  <Icon>search</Icon>
               </Button>
           </Grid>

           <Grid item xs={9}>
               <Table columns={columns} dataSource={listaProductMissing} rowKey="id" />
           </Grid>

         </Grid>
        
         <Modal
            title="Actualizar producto"
            centered
            visible={visible}
            onOk={() => actualizarProducto()}
            onCancel={() => setVisible(false)}
            width={1000}
          >
          
            <FormProduct formInstance={form}  hideModal={ () => setVisible(false)}  />
         </Modal>

         <Modal
             visible={visibleDelete}
             footer={[
               <Button key="2"
                   onClick={() =>setVisibleDelete(false)}
                 >No</Button>,
               <Button key="3" 
                   onClick={() => {
                      deleteItem(productDelete.id);
                     }}
                   type="primary">
                   Si
               </Button>
             ]}
          >
          
          <h2>¿Desea eliminar este producto solicitado?</h2>
          <p>{productDelete.description} </p>
       </Modal>

       <ProductMissing instanceDialog = { INSTANCE_PRODUCT_MISSING} />
    </div>
  );
}

export default ListProductMissing;
